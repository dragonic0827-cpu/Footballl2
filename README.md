...
Vercel 배포는 `pyproject.toml`의 `tool.vercel.entrypoint`가 지정한 루트 `app.py`에서 WSGI 앱을 발견한다. `app.py`는 HTTP 구현이 있는 `api/index.py`의 `app`과 `application`을 명시적으로 내보내므로, 설정 기반 탐색과 관례 기반 탐색을 모두 지원한다. 루트 경로에는 한국어 상태 화면이, `/api/health`에는 초기 세계를 실제로 생성해 확인한 JSON 상태가 반환된다. 로컬에서는 다음과 같이 실행할 수 있다.
